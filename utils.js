...
/**
 * Factor by which icon textures are oversampled so they stay sharp under
 * magnification. Follows the magnification cap and never drops below 2.
 *
 * @returns {number} the oversampling factor
 */
export function getIconOversample() {
    const maxScale =
        Docking.DockManager.settings?.magnifyMaxScale ?? 2;
    return Math.max(2, Math.ceil(maxScale));
}
