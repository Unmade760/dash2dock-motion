...
class DockDashItemContainer extends Dash.DashItemContainer {
    ...
    // Called by the magnifier each frame so a visible label keeps tracking
    // the magnified edge of its icon.
    updateLabelPosition() {
        if (this.label?.visible && this._labelText && this.label.get_stage())
            AppIcons.positionItemLabel(this);
    }
...
export const DockDash = GObject.registerClass({
    ...
    /**
     * Recreate every icon texture at the current oversampling factor. Called
     * when the magnification cap changes.
     */
    refreshIconResolution() {
        for (const actor of this._box.get_children()) {
            const baseIcon = actor.child?.icon;
            if (baseIcon)
                baseIcon._createIconTexture(baseIcon.iconSize);
        }
        const showAppsIcon = this._showAppsIcon?.icon;
        if (showAppsIcon)
            showAppsIcon._createIconTexture(showAppsIcon.iconSize);
    }
