...
const DashSlideContainer = GObject.registerClass({
    ...
    vfunc_allocate(box) {
        ...
        // Pad the slide clip so magnified icons can overflow the dock bounds,
        // both outwards past the slab and along the dock's axis as the strip
        // expands. The slide out crop still works because the padding only
        // opens area that is empty when nothing is magnified.
        const along = Magnifier.MAX_EFFECT_WIDTH;
        let padX1 = 0, padX2 = 0, padY1 = 0, padY2 = 0;
        switch (this.side) {
        case St.Side.BOTTOM:
            padX1 = padX2 = along;
            padY1 = childHeight * Magnifier.MAX_PERP_OVERFLOW + 8;
            break;
        case St.Side.TOP:
            padX1 = padX2 = along;
            padY2 = childHeight * Magnifier.MAX_PERP_OVERFLOW + 8;
            break;
        case St.Side.LEFT:
            padY1 = padY2 = along;
            padX2 = childWidth * Magnifier.MAX_PERP_OVERFLOW + 8;
            break;
        case St.Side.RIGHT:
            padY1 = padY2 = along;
            padX1 = childWidth * Magnifier.MAX_PERP_OVERFLOW + 8;
            break;
        }
        this.child.set_clip(
            -childBox.x1 - padX1,
            -childBox.y1 - padY1,
            -childBox.x1 + availWidth + padX1 + padX2,
            -childBox.y1 + availHeight + padY1 + padY2);
    }
...
const DockedDash = GObject.registerClass({
    ...
    _init(params) {
        ...
        // Pointer-tracking magnification engine
        this._magnifier = new Magnifier.DockMagnifier(
            this.dash, this._box, this._position, DockManager.settings);

        // Glass slab
        this._glassSlab = new Glass.GlassSlab(
            this.dash, this, DockManager.settings);
    ...
    _onDestroy() {
        ...
        this._glassSlab.destroy();
        this._glassSlab = null;
    ...
    _bindSettingsChanges() {
        ...
        this._signalsHandler.add([
            ...
            settings,
            'changed::running-apps-exclusions',
            () => {
                this.dash.resetAppIcons();
            },
        ], [
    ...
    /**
     * A positive edge distance floats the dock inwards as padding on the edge
     * side rather than as a position offset: the container keeps touching the
     * edge, so Mutter still builds a strut for it (layout.js drops the strut
     * entirely for an actor that does not reach the monitor border) and the
     * reserved area covers the dock plus the gap.
     *
     * A negative distance cannot be expressed as padding, so it is applied as
     * an outward position offset in _resetPosition() instead, pushing the dock
     * past the edge. The strut survives that because it is clamped to the
     * screen and the container still reaches the border.
     */
    _updateEdgeDistance() {
        const edgeDistance = DockManager.settings.edgeDistance;
        const side = {
        ...
        this.style = edgeDistance > 0 ? `padding-${side}: ${edgeDistance}px;` : null;
    }

    /**
     * The edge gap in physical pixels, as St scales CSS lengths by the
     * theme context scale factor.
     *
     * @returns {number} gap in physical pixels
     */
    _scaledEdgeDistance() {
        const {scaleFactor} = St.ThemeContext.get_for_stage(global.stage);
        return Math.round(DockManager.settings.edgeDistance * scaleFactor);
    }
    ...
    _resetPosition() {
        ...
        this._updateEdgeDistance();

        // Only the negative part moves the dock; positive gaps are padding.
        const overshoot = Math.min(this._scaledEdgeDistance(), 0);

        if (this._isHorizontal) {
            this.width = Math.round(fraction * workArea.width);

            let posY = this._monitor.y + overshoot;
            if (this._position === St.Side.BOTTOM)
                posY += this._monitor.height - 2 * overshoot;
            ...
            let posX = this._monitor.x + overshoot;
            if (this._position === St.Side.RIGHT)
                posX += this._monitor.width - 2 * overshoot;
    ...
    _updateStaticBox() {
        // For BOTTOM and RIGHT the box is measured back from the far edge,
        // which the edge distance padding does not move. TOP and LEFT already
        // pick the gap up through the slider's own offset. Only the padding
        // case needs correcting, as a negative distance is already included
        // in this.x and this.y.
        const gap = Math.max(this._scaledEdgeDistance(), 0);
        const gapX = this._position === St.Side.RIGHT ? gap : 0;
        const gapY = this._position === St.Side.BOTTOM ? gap : 0;

        this.staticBox.init_rect(
            this.x + this._slider.x - gapX -
                (this._position === St.Side.RIGHT ? this._box.width : 0),
            this.y + this._slider.y - gapY -
