...
const DockSettings = GObject.registerClass({
    ...
    // Built in code rather than in Settings.ui, which is left unmodified.
    _buildMagnificationPage() {
        ...
        const addScale = (key, title, subtitle,
            {lower, upper, stepIncrement, digits, mark}) => {
            ...
            const adjustment = new Gtk.Adjustment({
                ...
                page_increment: stepIncrement * 4,
            });
            ...
            // The label must be non-null: unlabelled marks make the marks
            // gizmo report a negative min width (Gtk-WARNING).
            scale.add_mark(mark, Gtk.PositionType.BOTTOM, ' ');
            this._settings.bind(key, adjustment, 'value',
                Gio.SettingsBindFlags.DEFAULT);

            page.append(titleLabel);
            page.append(subtitleLabel);
            page.append(scale);
        };

        addScale('magnify-max-scale',
            __('Magnification'),
            __('Scale of the icon under the cursor. 1.00 disables magnification.'),
            {lower: 1.0, upper: 3.0, stepIncrement: 0.05, digits: 2, mark: 2.0});
        addScale('magnify-effect-width',
            __('Effect width'),
            __('Span of the magnification effect along the dock, in pixels.'),
            {lower: 160, upper: 560, stepIncrement: 10, digits: 0, mark: 320});
        addScale('magnify-rise',
            __('Rise'),
            __('How far magnified icons lift off the dock, as a fraction of the icon size.'),
            {lower: 0.0, upper: 1.0, stepIncrement: 0.05, digits: 2, mark: 0.0});
        addScale('edge-distance',
            __('Distance from edge'),
            __('Gap between the dock and the screen edge it sits on, in pixels. 0 keeps it flush; negative values push it past the edge.'),
            {lower: -16, upper: 8, stepIncrement: 1, digits: 0, mark: 0});
        ...
        const addSwitch = (key, title, subtitle) => {
            ...
            this._settings.bind(key, toggle, 'active',
                Gio.SettingsBindFlags.DEFAULT);
            row.append(labels);
            row.append(toggle);
            page.append(row);
        ...
        addSwitch('show-icon-highlight',
            __('Icon highlight'),
            __('Show the translucent box behind hovered and focused icons.'));
        addSwitch('glass-outline',
            __('Glass outline'),
