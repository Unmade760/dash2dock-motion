...
export const DockAbstractAppIcon = GObject.registerClass({
    ...
    updateIconGeometry() {
        ...
        // The magnifier and the launch bounce transform this actor every
        // frame, so its transformed extents are not a stable minimize target:
        // a window event arriving while the dock is magnified would store a
        // laterally displaced rectangle. Walk the allocation chain up to the
        // dash instead, since allocations never include transforms.
        let baseX = 0, baseY = 0;
        let ancestor = this;
        while (ancestor && ancestor.name !== 'dash') {
            baseX += ancestor.allocation.x1;
            baseY += ancestor.allocation.y1;
            ancestor = ancestor.get_parent();
        }
        if (ancestor) {
            const [dashX, dashY] = ancestor.get_transformed_position();
            rect.x = Math.round(dashX + baseX);
            rect.y = Math.round(dashY + baseY);
            rect.width = Math.round(this.allocation.get_width());
            rect.height = Math.round(this.allocation.get_height());
        } else {
    ...
    // Load the texture at the magnification cap and pin the actor to the
    // stock footprint, so it is scaled down at rest and a magnified icon is
    // rendered from real pixels rather than upscaled by the GPU.
    _createIcon(iconSize) {
        const icon = this.app.create_icon_texture(
            iconSize * Utils.getIconOversample());
        icon.set_size(iconSize, iconSize);
        return icon;
    }

    // Bounce on the axis perpendicular to the magnifier's, so the two can run
    // at once. EASE_OUT_BOUNCE gives the diminishing rebounds.
    animateLaunch() {
        const position = Utils.getPosition();
        const {scaleFactor} = St.ThemeContext.get_for_stage(global.stage);
        const travel = (this.icon?.iconSize ?? 48) * 0.55 * scaleFactor;
        ...
        this.remove_transition(prop.replace(/_/g, '-'));
        this.ease({
            [prop]: dir * travel,
            duration: 200,
...
, class DockShowAppsIcon extends Dash.ShowAppsIcon {
    ...
    _createIcon(size) {
        this._iconActor = super._createIcon(size * Utils.getIconOversample());
        this._iconActor.set_size(size, size);
    ...
    vfunc_button_press_event(...args) {
        try {
            // Resolving this vfunc through the prototype can fail depending on
            // the shell version; propagate the event if it does.
...
class DockShowAppsIconMenu extends DockAppIconMenu {
    _rebuildMenu() {
        ...
        this.addMenuItem(new PopupMenu.PopupSeparatorMenuItem(__('Dash2Dock Motion')));
 ...
 */
export function positionItemLabel(item) {
    // Measure the transformed extents of the visual actor, item.child when
    // there is one, so the label follows the magnified edge of the icon
    // rather than its base layout box.
    const visual = item.child ?? item;
    const extents = visual.get_transformed_extents();
    const stageX = extents.get_x();
    const stageY = extents.get_y();
    const itemWidth = extents.get_width();
    const itemHeight = extents.get_height();

    const node = item.label.get_theme_node();
